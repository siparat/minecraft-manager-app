...
export const EditAppModal = ({ appData }: Props): JSX.Element => {
	...
	const onSubmit = async (data: FormValues): Promise<void> => {
		...
		try {
			await editApp(appData.id, data);
			toast.success('Приложение успешно отредактировано', { id: toastId });
			reset();
			navigate(0);
		} catch (error) {
			if (error instanceof HTTPError) {
	...
	return (
		<Portal>
			...
			<Content className="dialogContent">
				...
				<ContentBox className={styles['modal']} title="Редактировать приложение">
